...
class User extends Base{
	...
	public static function getAllUsers( $start ='' ,$page_size='' ) {
		...
		$db->query("SET @num=0");
		$db->query("UPDATE ".self::getTableName()." SET user_id=(@num:=@num+1)");

		
		$sql = "select * ,coalesce(g.group_name,'已删除') from ".self::getTableName()." u left join ".UserGroup::getTableName()." g on u.user_group = g.group_id order by u.user_id asc $limit";
