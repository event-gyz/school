...
class QuickNote extends Base {
	...
	public static function getNotes($start ='' ,$page_size='') {
		...
		// kidsdna希望每次都看到從1開始的編號... 
		$db->query("SET @num=0");
		$db->query("UPDATE ".self::getTableName()." SET note_id=(@num:=@num+1)");
	
		$sql="select ".self::$columns." ,coalesce(u.user_name,'已删除') as owner_name from ".self::getTableName()." q left join ".User::getTableName()." u on q.owner_id =  u.user_id order by q.note_id asc $limit";
