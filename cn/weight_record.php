...
if(isset($_POST['type']) && $_POST['type'] == 'diary'){
    ...
    if ($supervisor_uid = $CMEMBER->accessFromToken($_token)) {
        ...
        $sql = "INSERT INTO wap_weight (`date`,weight, picurl,uid) VALUES ('{$date}',$weight,'{$picurl}',$supervisor_uid)";
        $result = query($sql);
        if($result!=null) {
...
if(isset($_POST['type']) && $_POST['type'] == 'update'){
    ...
    if ($supervisor_uid = $CMEMBER->accessFromToken($_token)) {
        ...
        $sql = "update wap_weight set weight='{$weight}',picurl='{$picurl}',`date`='{$date}' where id=$id";
        $result = query($sql);
        if($result!=null) {
            header("Location:weight_record_list.php");
