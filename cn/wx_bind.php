...
if(!query_result($select)){
	...
	$row = M()->find($sql);
    if(empty($row)){
        die(genResponse(false, "该手机号未注册过账号"));
    }
	//判断手机号是否绑定过微信
	if(isset($row['wx_openid']) && !empty($row['wx_openid'])){
        die(genResponse(false, "该手机号已经绑定过微信，请修改手机号,或联系客服"));
	}
    ...
    $update = "update member set wx_openid='".$user_obj['openid']."' where uid =".$row['uid'];
    if(query($update)){
        // 更新手机验证码信息状态
