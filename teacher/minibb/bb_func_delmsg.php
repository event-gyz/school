...
Latest File Update: 2010-Aug-16
*/
...
//if($first!=$post) {
$post+=0;
if(!$onePost) { $rww=db_simpleSelect(0,$Tp,'poster_id','post_id','=',$post); $poster_id=$rww[0]; }
db_delete($Tp,'post_id','=',$post);
if($poster_id!=0 and !isset($pUsers[$poster_id])) {
$pUsers[$poster_id]=TRUE;
}
//}
if(file_exists($pathToFiles.'bb_plugins2.php')) require($pathToFiles.'bb_plugins2.php');
}// deletion cycle

if(!defined('ARCHIVE')) $arcId=''; else $arcId=ARCHIVE;
foreach($pUsers as $uid=>$t){
db_calcTotalUserAmount($uid, $Tu, $Tt, $Tp, $Taus, $arcId);
}

if(isset($archives)) db_calcTotalStats($arcId, $Tt, $Tp, $Tas);
...
$meta_relocate=addGenURLPage("{$main_url}/{$indexphp}action=vthread&forum={$forum}&topic={$topic}", $page, '&');
echo ParseTpl(makeUp($metaLocation));
exit;
}
else {
if(isset($mod_rewrite) and $mod_rewrite) $urlp=addTopicURLPage(genTopicURL($main_url, $forum, '#GET#', $topic, $topicData[0]), $page); else $urlp=addGenURLPage("{$main_url}/{$indexphp}action=vthread&forum={$forum}&topic={$topic}", $page, '&');
