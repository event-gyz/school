...
$result=mysql_query($sql);
if($result) return mysqli_affected_rows();
else return FALSE;
}
...
if($res and mysqli_num_rows($res)>0 and $row=mysql_fetch_row($res)) return $row[1]; else return FALSE;
}
...
if($res=mysql_query('select count(*) from '.$tbName.' where '.$tbKey.'='.$tbVal) and mysqli_num_rows($res)>0 and $row=mysql_fetch_row($res)) $amount=$row[0];
else $amount=0;
...
if($res=mysql_query("select count(*) from {$tbTopics} where topic_poster={$user_id}") and mysqli_num_rows($res)>0 and $row=mysql_fetch_row($res)) $num_live_topics=$row[0];
else $num_live_topics=0;

if($res=mysql_query("select count(*) from {$tbPosts} where poster_id={$user_id}") and mysqli_num_rows($res)>0 and $row=mysql_fetch_row($res)) $num_live_posts=$row[0];
...
if($res=mysql_query("select sum(num_topics), sum(num_posts) from {$tbArcStats} where user_id={$user_id} and archive_id!='{$archive}'") and mysqli_num_rows($res)>0 and $row=mysql_fetch_row($res)) { $otherArchivedTopics=$row[0]; $otherArchivedPosts=$row[1]; } else { $otherArchivedTopics=0; $otherArchivedPosts=0; }

$num_archived_topics=$num_live_topics+$otherArchivedTopics;
$num_archived_posts=$num_live_posts+$otherArchivedPosts;
$currentArchive=TRUE;

if($res=mysql_query("select {$GLOBALS['dbUserSheme']['num_live_topics'][1]}, {$GLOBALS['dbUserSheme']['num_live_posts'][1]} from {$tbUsers} where {$GLOBALS['dbUserId']}={$user_id}") and mysqli_num_rows($res)>0 and $row=mysql_fetch_row($res)) {
...
if(!$currentArchive and $res=mysql_query("select sum(num_topics), sum(num_posts) from {$tbArcStats} where user_id={$user_id}") and mysqli_num_rows($res)>0 and $row=mysql_fetch_row($res)) {
...
if($res=mysql_query("select count(*) from {$tbTopics}") and mysqli_num_rows($res)>0 and $row=mysql_fetch_row($res)) $GLOBALS['num_topics']=$row[0];
else $GLOBALS['num_topics']=0;

if($res=mysql_query("select count(*) from {$tbPosts}") and mysqli_num_rows($res)>0 and $row=mysql_fetch_row($res)) $GLOBALS['num_posts']=$row[0];
else $GLOBALS['num_posts']=0;

$GLOBALS['archive_id']=$prefix;

if($res=mysql_query("select archive_id from {$tbArcStats} where archive_id='{$prefix}'") and mysqli_num_rows($res)>0){
